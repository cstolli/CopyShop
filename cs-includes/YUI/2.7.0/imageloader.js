...
function yuiPDFUploader(rte, editor_name, upload_url, upload_pdf_name) {
   // customize the editor img button 


   //YAHOO.log( "Adding Click Listener" ,'debug');
   rte.addListener('toolbarLoaded',function() {
       rte.toolbar.addListener ( 'insertpdfClick', function(o) {
           try {
               var pdfPanel=new YAHOO.util.Element(editor_name + '-panel');
               pdfPanel.on ( 'contentReady', function() {
                   try {
                       var Dom=YAHOO.util.Dom;

                       if (! Dom.get(editor_name + '_insertpdf_upload'))
                       {
                           var label=document.createElement('label');
                          label.innerHTML='<strong>Upload:</strong>'+
			         '<input type="file" id="' +
				  editor_name + '_insertpdf_upload" name="'+upload_pdf_name+
			         '" size="10" style="width: 300px" />'+
			         '</label>';

                           var img_elem=Dom.get(editor_name + '_insertpdf_url');
                           Dom.getAncestorByTagName(pdf_elem, 'form').encoding = 'multipart/form-data';

                           Dom.insertAfter(
                               label,
                               pdf_elem.parentNode);

                           YAHOO.util.Event.on ( editor_name + '_insertpdf_upload', 'change', function(ev) {
                               YAHOO.util.Event.stopEvent(ev); // no default click action
                               YAHOO.util.Connect.setForm ( img_pdf.form, true, true );
							  // alert('attempting async');
                               var c=YAHOO.util.Connect.asyncRequest(
                               'POST', upload_url, {
                                   upload:function(r){
                                       try {
										   ...
										   alert(o.status);
                                           if (o.status=='UPLOADED') {
                                               Dom.get(editor_name + '_insertpdf_upload').value='';
                                               Dom.get(editor_name + '_insertpdf_url').value=o.pdf_url;
                                               // tell the image panel the url changed
                                               // hack instead of fireEvent('blur')
                                               // which for some reason isn't working
                                               Dom.get(editor_name + '_insertpdf_url').focus();
                                               Dom.get(editor_name + '_insertpdf_upload').focus();
                                           } else {
                                               alert ( "Upload Failed: "+o.status );
                                           }
