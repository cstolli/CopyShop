...
function FIC_checkForm(e) {
	...
	for (i=0;i<f_in.length;i++) {
		if (f_in[i].type.toLowerCase() != 'submit' && f_in[i].type.toLowerCase() != 'button' && f_in[i].type.toLowerCase() != 'hidden') {
			if (isVisible(f_in[i])) {
				...
				if (valid) {
					removeClassName(f_in[i],'validation-failed');
					addClassName(f_in[i],'validation-passed');
				} else {
					removeClassName(f_in[i],'validation-passed');
					addClassName(f_in[i],'validation-failed');
					//try to get title
	...
	if (!all_valid) {
		...
		var fails = YAHOO.util.Selector.query('.validation-failed');
...
function FIC_checkField(c,e) {
	...
	//search for required with default text still in it
	if (c.indexOf(' required ') != -1 && t == tt){
		//required found, but field still has default tip text in it
		valid = false;
	}
	...
	if (c.indexOf(' required ') != -1){
		...
		//var n = e.getAttribute('greaterthan');
		//alert(e.value);
		//alert(Math.abs(n));
		//x = Math.abs(n);
		//alert(x);
	...
	if (c.indexOf(' validate-number ') != -1 && isNaN(t) && t.match(/[^\d]/)) {
		...
		//digit bad
		valid = false;
	} else if (c.indexOf(' validate-alpha ') != -1 && !t.replace(/ /,'').match(/^[a-zA-Z]+$/)) {
		//alpha bad
		valid = false;
	...
	} else if (c.indexOf(' validate-phone ') != -1 && !t.match(/1?\s*\W?\s*([2-9][0-8][0-9])\s*\W?\s*([2-9][0-9]{2})\s*\W?\s*([0-9]{4})(\se?x?t?(\d*))?/)) {
		//email bad
		valid = false;
		if (c.indexOf(' required ') == -1 && t.length == 0) {
			valid = true;
