 ...
 $id = $_POST["fid"];
	 ...
	 // $image = $images->fetch_row();
	  ...
	  //if (!$image) {
		...
		move_uploaded_file($file['tmp_name'], "../" .CS_SITEPATH."/uploads/" . $category . "/" . $file['name']);  
		...
		$sql = "insert into cs_uploads (fid, ftable, filename, mediumsize, thumbnail, category) values ($id, '" . $table . "', '" . $file['name'] . "', '" . $file['name'] . "', '" . $file['name'] . "', '" .$category . "')";
