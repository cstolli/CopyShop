<div id="cs_nav">
<?
$menu = $mysqli->query("select cs_menu_items.id, target, label, `order`, href, title from cs_menu_items inner join cs_menus on cs_menus.id = cs_menu_items.menu where cs_menus.name='Main Navigation' order by `order` asc");
//echo $mysqli->error;
$x = 1;
$width = 0;
...
$menuItem = $menu->fetch_assoc();
	...
	if (strtolower($pg) == strtolower(str_replace(".html", "", $menuItem["href"]))) {$class="active";} else {$class="normal";}
